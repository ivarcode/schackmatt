...
<div id="container">
	...
	<div id="content">
		...
		<?php
			...
			// if the $url contains game_did_not_load_properly error
			if (strpos($url,'error=game_did_not_load_properly') !== false) {
				echo "<br><h3 class='red_error_text'>The game has not loaded properly, it is possible it no longer exists on the server, or you have a bad url.</h3>";

			} else {
				$url = substr($url, -32);
				// echo $url;
				$sql = "SELECT * FROM games WHERE url='$url'";
				$result = mysqli_query($conn,$sql);
				if (mysqli_num_rows($result) == 0) {
					// echo "should be happenin";
					$rdir = "Location: ./game.php?url=".$url."?error=game_did_not_load_properly";
					header($rdir);
					exit();
				}
				...
				$rslt = $row['result'];
				$pgn = $row['pgn'];
				// echo $white.$black.$type.$rslt.$pgn;
				$board_orientation = null;
				if (isset($_SESSION['id'])) {
					...
					$result = mysqli_query($conn,$sql);
					$row = mysqli_fetch_assoc($result);
					if ($row['username'] == $white) {
						$board_orientation = 'white';
					} else if ($row['username'] == $black) {
						$board_orientation = 'black';
					}

				} else {
					// default guest view page
					$board_orientation = 'white';
				}
				$self = null;
				$opponent = null;
				if ($board_orientation == 'white') {
					$opponent = $black;
					$self = $white;
				} else if ($board_orientation == 'black') {
					$opponent = $white;
					$self = $black;
				}
				$pgn_html = "";
				echo "<br><div id='game'><canvas id='board' width='640' height='640'>canvas</canvas>
				<div id='game_result'>
				...
				<p id='board_orientation'>".$board_orientation."</p>
				<p id='white'>".$white."</p>
				<p id='black'>".$black."</p>
				<p id='type'>".$type."</p>
				<p id='pgn'>".$pgn."</p>
		...
		<script type="text/javascript">
			...
			var white_player = "<?php echo $white ?>";
			var black_player = "<?php echo $black ?>";
			var orientation = "<?php echo $board_orientation ?>";
			var board_canvas;
			var board_context;
			var SQ_DIM = 80;
			...
			var game = new Game(document.getElementById("white").innerHTML,document.getElementById("black").innerHTML,document.getElementById("type").innerHTML,"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1",[]);
			...
			function setup() {
				...
				board_canvas.addEventListener('mousedown',function(events){
					...
					if (events.button === 0 /*left mousebutton pressed*/) {
						...
						strategic_draws[strategic_draws.length] = {src:get_sq_from_mouse_pos(get_mouse_pos(board_canvas,events)),dest:null};
					}
				...
				board_canvas.addEventListener('mouseup',function(events){
					...
					if (strategic_draws.length > 0 && strategic_draws[strategic_draws.length-1].src != null && strategic_draws[strategic_draws.length-1].dest == null) {
						...
						if (move_is_legal && !promoting) {
							...
							if (orientation == 'black') {
								move_data.dest.x = 7-move_data.dest.x;
								move_data.dest.y = 7-move_data.dest.y;
								promoting_to.x = 7-promoting_to.x;
								promoting_to.y = 7-promoting_to.y;
								console.log(move_data);
								console.log(promoting_to);
								if (promoting_to.x == 7) {
			...
			function click(src,dest) {
				/*responsible for handling events after valid 'clicks' have occured on board, called from mouseup listener*/
				console.log("click @ "+src.x+","+src.y+" -> "+dest.x+","+dest.y);
			}
			...
			function draw_board() {
				...
				if (selected_square != null && mousedown) {
					...
					if (orientation == 'black') {
						draw_piece(560-(current_mouse_pos.x-40),560-(current_mouse_pos.y-40),get_piece(board,selected_square));
					} else {
				...
				if (promoting) {
					if (orientation == 'black') {
						if (promoting_to.x == 7) {
							...
							draw_piece((7-promoting_to.y)*SQ_DIM,promoting_to.x*SQ_DIM,'q');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x*SQ_DIM)-SQ_DIM,'n');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x*SQ_DIM)-(SQ_DIM*2),'r');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x*SQ_DIM)-(SQ_DIM*3),'b');
							...
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x+7)*SQ_DIM,'Q');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x+6)*SQ_DIM,'N');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x+5)*SQ_DIM,'R');
							draw_piece((7-promoting_to.y)*SQ_DIM,(promoting_to.x+4)*SQ_DIM,'B');
						}
					} else {
						if (promoting_to.x == 7) {
							tint_sq(promoting_to.y,(7-promoting_to.x),"green",0.8);
				...
				if (mouse_over_board) {
					...
					for (var i = 0; i < m.length; i++) {
						...
						if (m[i].src.rank == cms.x && m[i].src.file == cms.y) {
							if (orientation == 'black') {
								draw_move_arrow({rank:7-m[i].src.rank,file:7-m[i].src.file},{rank:7-m[i].dest.rank,file:7-m[i].dest.file});
							} else {
			...
			function draw_move_arrow(src,dest) {
				...
				// console.log("draw move arrow",src,dest);
